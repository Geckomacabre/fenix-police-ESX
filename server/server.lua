...
if Config.Framework == 'ESX' then
    ESX = exports['es_extended']:getSharedObject()  -- ← This is the modern, correct way
...
CreateThread(function()
    while true do
        local polCount = 0
        if Config.Framework == 'ESX' then
            local xPlayers = ESX.GetExtendedPlayers()  -- Use GetExtendedPlayers for modern ESX
                ...
                for _, job in ipairs(Config.PoliceJobsToCheck) do
                    if xPlayer.job.name == job.jobName then
                        ...
                        if job.onDutyOnly then
                            if xPlayer.job.onDuty then  -- Assuming your ESX job system has onDuty field (common in esx_policejob etc.)
                                polCount = polCount + 1
                            end
                        else
                            polCount = polCount + 1
...
AddEventHandler('spawnPoliceHeliNet', function(wantedLevel, playerCoords, spawnPoint, spawnHeading, spawnTable)
    local src = source
    local seatIndex = -1
    ...
    while (not vehicleCrewed) and hasDriverCount < Config.hasDriverWaitCount do
        ...
        local vehicle = CreateVehicleServerSetter(vehicleHash, 'heli', spawnPoint.x, spawnPoint.y, spawnPoint.z, spawnHeading)
        local waitCount = 0
        while not DoesEntityExist(vehicle) and waitCount < Config.spawnWaitCount do
            Wait(10)
            waitCount = waitCount + 1
        ...
        for _, pedModel in ipairs(selectedEntry.pilots) do
            ...
            while (not pedInSeat or pedInSeat == 0) and warpCount < Config.warpWaitCount do
                ...
                if (not pedInSeat or pedInSeat == 0) then
                    ...
                    local pedNetID = NetworkGetNetworkIdFromEntity(officer)
                    if Config.isDebug then print('NET ped ' ..pedModel .. ' spawned with pedNetID = ' ..pedNetID .. ' for vehNetID = ' .. vehNetID) end
                    -- Give pilot loadout (fixed to use proper loadout instead of hardcoded)
...
AddEventHandler('spawnPoliceAirNet', function(wantedLevel, playerCoords, spawnPoint, spawnHeading, spawnTable)
    local src = source
    local seatIndex = -1
    -- Variable will be set true as soon as a vehicle has a driver. I'm less worried about a crew member not warping in properly.
    local vehicleCrewed = false
    ...
    while (not vehicleCrewed) and hasDriverCount < Config.hasDriverWaitCount do
        ...
        local vehicleHash = GetHashKey(selectedEntry.unit.model)
        local vehicle = CreateVehicleServerSetter(vehicleHash, 'plane', spawnPoint.x, spawnPoint.y, spawnPoint.z, spawnHeading)
        local waitCount = 0
        while not DoesEntityExist(vehicle) and waitCount < Config.spawnWaitCount do
            Wait(10)
            waitCount = waitCount + 1
